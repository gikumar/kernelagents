# backend/app/visualization_service.py
import json
import logging
...
class VisualizationService:
    ...
    def __init__(self):
        logger.info("💥 VisualizationService initializing")
    ...
    def _detect_chart_type_from_query(self, query: str, data: List[Dict]) -> str:
        ...
        logger.info(f"💥 _detect_chart_type_from_query called with query: '{query[:50]}...' and {len(data)} data rows")
        
        if not VISUALIZATION_AVAILABLE:
            logger.info("💥 Visualization not available, defaulting to bar chart")
            return "bar"
        ...
        logger.info("💥 Starting keyword-based chart type detection")
        
        trend_keywords = ["trend", "over time", "history", "timeline", "growth"]
        if any(word in query_lower for word in trend_keywords):
            logger.info(f"💥 Identified trend keywords: {[w for w in trend_keywords if w in query_lower]}")
            logger.info("💥 Chart type detected: bar (trend)")
            return "bar" 
        ...
        if any(word in query_lower for word in distribution_keywords):
            logger.info(f"💥 Identified distribution keywords: {[w for w in distribution_keywords if w in query_lower]}")
            logger.info("💥 Chart type detected: pie")
            return "pie"
        ...
        if any(word in query_lower for word in comparison_keywords):
            logger.info(f"💥 Identified comparison keywords: {[w for w in comparison_keywords if w in query_lower]}")
            logger.info("💥 Chart type detected: bar (comparison)")
            return "bar"
        
        scatter_keywords = ["relationship", "correlation", "scatter"]
        if any(word in query_lower for word in scatter_keywords):
            logger.info(f"💥 Identified scatter keywords: {[w for w in scatter_keywords if w in query_lower]}")
            logger.info("💥 Chart type detected: scatter")
            return "scatter"
        
        # Default to bar chart if no specific type is detected but data is present
        if data and len(data) > 0:
            logger.info("💥 No specific keywords found, defaulting to bar chart due to data presence")
            return "bar"
        
        # Data-based detection
        logger.info("💥 Starting data-based chart type detection")
        if data and len(data) > 0:
            ...
            try:
                ...
                if date_columns and numeric_columns:
                    logger.info("💥 Chart type detected: line (date + numeric columns)")
                    return "line"
                elif len(numeric_columns) >= 2:
                    logger.info("💥 Chart type detected: scatter (multiple numeric columns)")
                    return "scatter"
                elif len(data) <= 10:  # Small number of categories
                    logger.info(f"💥 Small dataset ({len(data)} rows), checking for pie chart suitability")
                    return "pie" if len(numeric_columns) == 1 else "bar"
            except Exception as e:
                logger.error(f"💥 Error during data-based detection: {str(e)}")
        
        logger.info("💥 Final fallback: defaulting to bar chart")
        return "bar"
    ...
    def generate_chart(self, data: List[Dict], query: str, title: str = "") -> Optional[Dict]:
        ...
        logger.info(f"💥 generate_chart called with {len(data)} rows, query: '{query[:30]}...', title: '{title}'")
        ...
        if not VISUALIZATION_AVAILABLE:
            logger.warning("💥 Visualization libraries not available, cannot generate chart")
            return None
        ...
        try:
            ...
            if len(df) > 5:
                logger.info("💥 Rotating x-axis labels for better readability")
            ...
            logger.info(f"💥 Chart generation successful, type: {chart_type}, title: '{title}'")
            return result
            
        except Exception as e:
            logger.error(f"💥 Error generating chart: {str(e)}")
            logger.error(f"💥 Chart generation failed for query: '{query}'")
            return None
    
    def _create_bar_chart(self, df: pd.DataFrame, ax: plt.Axes, query: str):
        """Create bar chart optimized for trading data"""
        logger.info("💥 _create_bar_chart called")
        ...
        for col in trading_value_cols:
            if col in df.columns:
                value_col = col
                logger.info(f"💥 Found value column: {value_col}")
                break
        
        if not value_col and numeric_cols:
            value_col = numeric_cols[0]
            logger.info(f"💥 Using first numeric column as value: {value_col}")
        ...
        if not label_col:
            ...
            logger.info("💥 No label column found, using index-based labels")
        else:
            labels = df[label_col].astype(str)
            values = df[value_col] if value_col else df[numeric_cols[0]]
            logger.info(f"💥 Using '{label_col}' as labels and '{value_col}' as values")
    ...
    def _create_line_chart(self, df: pd.DataFrame, ax: plt.Axes, query: str):
        ...
        logger.info("💥 _create_line_chart called")
        
        numeric_cols = df.select_dtypes(include=['number']).columns.tolist()
        logger.info(f"💥 Numeric columns found: {numeric_cols}")
        
        if not numeric_cols:
            logger.warning("💥 No numeric columns found for line chart")
            return
        ...
        if date_cols:
            ...
            logger.info("💥 No date columns found, using index as x-values")
            x_values = range(len(df))
    ...
    def _create_pie_chart(self, df: pd.DataFrame, ax: plt.Axes, query: str):
        """Create pie chart"""
        logger.info("💥 _create_pie_chart called")
        ...
        if numeric_cols and categorical_cols:
            ...
            logger.info(f"💥 Using value column: {value_col}, label column: {label_col}")
            
            # Take top 8 categories for readability
            df_sorted = df.sort_values(by=value_col, ascending=False).head(8)
            logger.info(f"💥 Using top {len(df_sorted)} categories for pie chart")
            ...
            logger.warning("💥 Insufficient data for pie chart - need both numeric and categorical columns")
    
    def _create_scatter_chart(self, df: pd.DataFrame, ax: plt.Axes, query: str):
        """Create scatter chart"""
        logger.info("💥 _create_scatter_chart called")
        ...
        if len(numeric_cols) >= 2:
            ...
            logger.info(f"💥 Using x-column: {x_col}, y-column: {y_col}")
            ...
            logger.warning("💥 Insufficient numeric columns for scatter chart (need at least 2)")
    
    def _generate_chart_title(self, query: str, chart_type: str) -> str:
        """Generate appropriate chart title from query"""
        logger.info(f"💥 _generate_chart_title called with query: '{query[:30]}...', chart_type: {chart_type}")
        
        query_lower = query.lower()
        
        if "top" in query_lower or "best" in query_lower:
            title = f"Top Results: {query}"
            logger.info(f"💥 Generated top/best title: '{title}'")
            return title
        elif "trend" in query_lower:
            title = f"Trend Analysis: {query}"
            logger.info(f"💥 Generated trend title: '{title}'")
            return title
        elif "distribution" in query_lower:
            title = f"Distribution: {query}"
            logger.info(f"💥 Generated distribution title: '{title}'")
            return title
        else:
            title = f"Visualization: {query}"
            logger.info(f"💥 Generated default title: '{title}'")
            return title

    def _extract_data_from_result(self, result_text: str) -> List[Dict]:
        """Extract structured data from the result text"""
        logger.info(f"💥 _extract_data_from_result called with text length: {len(result_text)}")
        ...
        try:
            ...
            logger.info(f"💥 Extracted {len(data)} data rows from result text")
            return data
